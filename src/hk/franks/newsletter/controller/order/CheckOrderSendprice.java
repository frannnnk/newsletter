package hk.franks.newsletter.controller.order;
...
public class CheckOrderSendprice extends ActionSupport {
	...
	/*	logger.info("判断是否达到起送价格");
		...
		if (orderstr != null && !orderstr.equals("")) {
			restaurantids = hk.franks.newsletter.logic.getRestaurantids(orderstr);
			List resultListArray[] = hk.franks.newsletter.logic.wraptoOrderFoodlist(orderstr,
					restaurantsStr);
			restaurantNameList = resultListArray[0];
			restaurantFoodList = resultListArray[1];
			orderSummoeyList = hk.franks.newsletter.logic.getOrderSumMoney(restaurantFoodList);

			// 如果订单都达到起送价格就进入订单确认界面
			if (hk.franks.newsletter.logic.checkisSatisfySendprice(restaurantids, orderSummoeyList)) {
